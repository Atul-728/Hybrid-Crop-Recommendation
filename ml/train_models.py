...
def train_xgboost(X_train, y_train):
    model = XGBClassifier(n_estimators=400, learning_rate=0.05, max_depth=6, random_state=42, eval_metric="mlogloss")
    model.fit(X_train, y_train)
...
def train_catboost(X_train, y_train):
    model = CatBoostClassifier(iterations=400, learning_rate=0.05, depth=6, random_seed=42, verbose=0)
    model.fit(X_train, y_train)
    joblib.dump(model, os.path.join(ARTIFACTS_PATH, "model_catboost.pkl"))
    return model

# --- NEW: Deep Tabular & Uncertainty Models ---

def train_tabnet(X_train, y_train, X_val, y_val):
    print("Training TabNet...")
    model = TabNetClassifier(optimizer_fn=torch.optim.Adam, optimizer_params=dict(lr=2e-2), verbose=0)
    model.fit(X_train, y_train, eval_set=[(X_val, y_val)], patience=10, max_epochs=100)
...
def train_ngboost(X_train, y_train):
    ...
    # NGBoost for classification requires specifying the number of classes (k)
    k = len(np.unique(y_train))
    model = NGBClassifier(Dist=k_categorical(k), n_estimators=100, learning_rate=0.05, random_state=42, verbose=False)
    model.fit(X_train, y_train)
...
def run_training():
    ...
    tabnet_model = train_tabnet(X_train, y_train, X_val, y_val)
    evaluate_model(tabnet_model, X_test, y_test, "tabnet")

    ngboost_model = train_ngboost(X_train, y_train)
    evaluate_model(ngboost_model, X_test, y_test, "ngboost")

    print("All base and deep models trained successfully.")
